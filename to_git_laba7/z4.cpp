//Лабораторная 7, задача 4. Выполнена: Прескурел Я.Ю.
...
Предусмотреть ввод положительных и отрицательных чисел.
	Вариант 3: в девятнадцатеричной*/
...
double toTen(char* input, int lenght_input)
{
    ...
    for (; i < lenght_input; ++i, --step) {
        ...
        if (input[i] == 'E' || input[i] == 'e') {
            answer += 14 * pow(14, step);
            continue;
        }
        if (input[i] == 'F' || input[i] == 'f') {
            answer += 15 * pow(14, step);
            continue;
        }
        if (input[i] == 'G' || input[i] == 'g') {
            answer += 16 * pow(14, step);
            continue;
        }
        if (input[i] == 'H' || input[i] == 'h') {
            answer += 17 * pow(14, step);
            continue;
        }
        if (input[i] == 'I' || input[i] == 'i') {
            answer += 18 * pow(14, step);
            continue;
        }
    }
...
int main()
{
    ...
    cout << "Данная программа осуществляет сложение и вычитание двух чисел в девятнадцатеричной системе счисления.";
